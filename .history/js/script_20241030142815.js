...
function displayMovies(movies) {
    ...
    if (movies.length === 0) {
        // Si no hay resultados, muestra el mensaje de "no se encontraron películas"
        const noResultsMessage = document.createElement("li");
        noResultsMessage.className = "list-group-item bg-dark text-white text-center";
        noResultsMessage.textContent = "Ups! No hemos encontrado ninguna película. ¡Vuelve a intentarlo!";
        lista.appendChild(noResultsMessage);
    } else {
        // Si hay resultados, elimina el mensaje de error previo

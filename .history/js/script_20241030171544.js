...
function displayMovies(movies) {
    ...
    if (movies.length === 0) {
        lista.classList.remove("dots"); // Quitar el borde si no hay películas

        const noResultsMessage = document.createElement("li");
        noResultsMessage.className = "list-group-item bg-dark text-white text-center no-results-message";
        noResultsMessage.innerHTML = `
            <h3><strong>¡Ups! No hemos encontrado ninguna película. ¡Vuelve a intentarlo!</strong></h3>
        `;
        lista.appendChild(noResultsMessage);
    } else {
        lista.classList.add("dots"); // Agregar borde cuando haya resultados

        movies.forEach(movie => {
            const listItem = document.createElement("li");
            listItem.className = "list-group-item bg-dark text-white dots"; // Clase dots en cada película
            listItem.innerHTML = `
                <h5>${movie.title}</h5>

...
function displayMovies(movies) {
    ...
    if (movies.length === 0) {
        ...
        noResultsMessage.innerHTML = `
            <h3><strong>¡Ups! No hemos encontrado ninguna película.</strong></h3> <br> <h3><strong>¡Vuelve a intentarlo!</strong></h3>
        `;
        lista.appendChild(noResultsMessage);
    } else {
        movies.forEach(movie => {
            const listItem = document.createElement("li");
            listItem.className = "list-group-item bg-dark text-white double";
            listItem.innerHTML = `
                <h5>${movie.title}</h5>
